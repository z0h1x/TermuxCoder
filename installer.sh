...
# Auto-fetch latest Code Server version
CS_VERSION=$(curl -s https://api.github.com/repos/coder/code-server/releases/latest | grep -oP '"tag_name": "\Kv[0-9.]+' | sed 's/v//')

# If curl fails, fallback to hardcoded version
CS_VERSION=${CS_VERSION:-"4.111.0"}
